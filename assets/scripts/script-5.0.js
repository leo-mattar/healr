...
function pricingTabber() {
  ...
  tabberPanels.forEach(panel => {
    const checkmarkBtn = panel.querySelector(".c-icon-wrap.checkmark");
    if (!checkmarkBtn) return;

    checkmarkBtn.addEventListener("click", function () {
      // Only proceed if this panel is currently active
      if (!panel.classList.contains("is-active")) return;

      // Toggle checkmark icons in active panel only
      const checkmarkIcons = panel.querySelectorAll(
        ".c-icon.tabber-checkmark-pricing"
      );
      checkmarkIcons.forEach(icon => icon.classList.toggle("is-active"));

      // Toggle all pricing texts in active panel
      const pricingTexts = panel.querySelectorAll(
        ".t-display-3.tabber-pricing"
      );
      pricingTexts.forEach(pricingText =>
        pricingText.classList.toggle("is-active")
      );

      // Toggle the closest savings element
      const savingsElement = panel.querySelector(".c-tabber-saving");
      if (savingsElement) {
        savingsElement.classList.toggle("is-active");
      }
    });
  });
}
